...
void testarErros(void*) {
  Serial.println("DIAGNOSTICO: erros de rede e OTA, sem substituir o teste principal.");
  Serial.println("TESTE 1: SEM WI-FI");
  REDE = "REDE-INEXISTENTE-CP2";
  conectarWiFi();
  REDE = "Wokwi-GUEST";
  const char* casos[] = {
    "https://raw.githubusercontent.com/luc916/Motiva-CP2-OTA/main/tests/ota/inexistente.json",
    "https://raw.githubusercontent.com/luc916/Motiva-CP2-OTA/main/tests/ota/atual.json",
    "https://raw.githubusercontent.com/luc916/Motiva-CP2-OTA/main/tests/ota/bin-ausente.json",
    "https://raw.githubusercontent.com/luc916/Motiva-CP2-OTA/main/tests/ota/bin-invalido.json"
  };
  for (int i=0; i<4; i++) {
    Serial.printf("\nTESTE %d: %s\n", i+2, casos[i]);
    MANIFESTO_URL = casos[i];
    consultarAtualizacao();
    delay(1000);
  }
  Serial.println("DIAGNOSTICO CONCLUIDO: verificar cada mensagem acima.");
  vTaskDelete(nullptr);
}
...
void setup() {
  ...
  anunciarSessao();
  xTaskCreate(testarErros, "testes", 12288, nullptr, 1, nullptr);
}
