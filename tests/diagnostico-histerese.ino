...
const uint32_t INTERVALO_SESSAO = 12000;
...
void concluirSessao() {
  ...
  sessoesConcluidas++;
  Serial.println("Sessao concluida. DIAGNOSTICO: proximo inicio em 12 s apos o anterior.");
}
...
void atualizarLeituras() {
  ...
  if (indice < QUANTIDADE &&
    ...
    // Apenas neste diagnostico: casos fixos e sessoes aceleradas para 12 s.
    const int casos[4][5] = {{15,14,15,16,15}, {18,16,15,16,20},
                            {15,14,15,16,15}, {14,10,20,12,14}};
    leituras[indice] = casos[sessoesConcluidas % 4][indice];
...
void setup() {
  Serial.begin(115200);
  Serial.println("DIAGNOSTICO HISTERese: dados fixos; nao comprova OTA nem periodo de 48 s.");
